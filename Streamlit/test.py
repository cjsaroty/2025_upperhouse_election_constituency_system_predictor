...
# ---------------------------
    ...
    if df is None:
        ...
        st.subheader("数値列ヒストグラム")
        num_cols = ["年齢", "衆参当選回数", "参議院当選回数", "衆議院当選回数", "政府規模"]
        chosen_num = st.selectbox("数値列を選択", options=num_cols)
        fig, ax = plt.subplots(figsize=(8, 4))
        sns.histplot(df[chosen_num].dropna(), kde=True, ax=ax)
        st.pyplot(fig)

        st.subheader("カテゴリ列別当選率")
        cat_cols = ["性別","党派","元現新","都道府県","争点1位","争点2位","争点3位",
                    "出生地外立候補フラグ","秘書経験フラグ","地方議会経験フラグ","職業(分類)"]
