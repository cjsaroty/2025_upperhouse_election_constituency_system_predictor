...
plt.rcParams["font.family"] = "MS Gothic"
sns.set_style("whitegrid")
shap_option = st.sidebar.checkbox("SHAPを表示する", key="show_shap")
...
# ---------------------------
# ---------------------------
# Train Model  → machine_learning の学習済みモデルに差し替え
# ---------------------------
elif page == "Train Model":
    st.title("Train Model（外部モデル読込モード）")

    st.info("このアプリでは、Streamlit 上で学習せず、machine_learning フォルダ内の既存モデルを利用します。")

    MODEL_PATH = "machine_learning/my_trained_model.pkl"

    if not os.path.exists(MODEL_PATH):
        st.error("machine_learning フォルダに my_trained_model.pkl が見つかりません。")
        st.stop()

    if st.button("モデルを読み込む"):
        with st.spinner("model.pkl を読み込んでいます..."):
            mdl = joblib.load(MODEL_PATH)

        # モデル要素を session_state に格納
        st.session_state["model"] = mdl["model"]
        st.session_state["features"] = mdl["features"]
        st.session_state["label_encoders"] = mdl.get("label_encoders", {})
        st.session_state["cbe"] = mdl.get("cbe", None)

        st.success("モデル読込完了！ Candidate Prediction ページで利用できます。")
