...
# ============================
# ユーザー入力による予測（LightGBM対応 完全版）
# ============================
print("\n*** 当選確率予測アプリ（16特徴量＋CatBoostEncoding） ***")
...
X_input = pd.DataFrame(
    [[
        age, gender, party_encoded, status_encoded, senkyo_all,
        sangiin, shugiin, seats, issue1_encoded, issue2_encoded,
        issue3_encoded, gov_scale, birth_flag, secretary_flag,
        local_flag, job_encoded
    ]],
    columns=features
)

# ----------------------------
# CatBoostEncoder 列作成（学習時と同じ順序で6列）
# ----------------------------
X_input_label = pd.DataFrame(
    [[party_encoded, status_encoded, issue1_encoded, issue2_encoded, issue3_encoded, job_encoded]],
    columns=label_cols
)
X_input_cbe = cbe.transform(X_input_label)

for col in label_cols:
    X_input[f"{col}_cbe"] = X_input_cbe[col].values
