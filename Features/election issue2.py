...
# 欠損除外（安全対策）
df = df.dropna(subset=["争点2位", "当落フラグ"])

# ==================================================
# 3. 争点2位ごとの当選確率・人数
# ==================================================
issue1_stats = (
    df.groupby("争点2位")["当落フラグ"]
      .agg(["mean", "count"])
...
ax.set_ylim(0, 1.05)
ax.set_xlabel("争点2位の政策", fontsize=12)
ax.set_ylabel("当選確率 (%)", fontsize=12)
ax.set_title("争点2位と当選確率の関係", fontsize=14, pad=15)
...
# 5. カイ二乗検定（争点2位 × 当落）
# ==================================================
contingency = pd.crosstab(df["争点2位"], df["当落フラグ"])
...
alpha = 0.05
if p < alpha:
    print("→ 有意差あり（争点2位と当選は独立ではない）")
else:
    print("→ 有意差なし（争点2位と当選は独立とみなせる）")
