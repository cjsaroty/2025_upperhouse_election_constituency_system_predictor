...
# 3. クロス集計（衆議院当選回数 × 当落）
# ==================================================
cross_tab = pd.crosstab(df["衆議院当選回数"], df["当落"])
...
alpha = 0.05
if p < alpha:
    print("→ 有意差あり（衆議院当選回数と当落は独立ではない）")
else:
    print("→ 有意差なし（衆議院当選回数と当落は独立とみなせる）")
...
# 5. 当選率の計算（可視化用）
# ==================================================
win_rate_by_count = df.groupby("衆議院当選回数")["当落フラグ"].mean().sort_index()
